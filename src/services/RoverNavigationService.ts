...
class RoverNavigationService {
  ...
  private static readonly cardinalDirectionMap = new Map([
    [CardinalDirections.North, { left: CardinalDirections.West, right: CardinalDirections.East }],
    [CardinalDirections.East, { left: CardinalDirections.North, right: CardinalDirections.South }],
    [CardinalDirections.South, { left: CardinalDirections.East, right: CardinalDirections.West }],
    [CardinalDirections.West, { left: CardinalDirections.South, right: CardinalDirections.North }],
  ]);
  ...
  private move() {
    ...
    if (!movement) {
      throw new Error('Invalid direction: Rover cannot move in that direction.');
    }

    const newX = this._rover.position.x + movement.x;
    const newY = this._rover.position.y + movement.y;

    const newRoverPosition = new Position(newX, newY, this._rover.position.cardinalDirection);
    const isRoverOffLimits = this._plateau.isOffLimits(newRoverPosition);

    if (isRoverOffLimits) {
      throw new Error('Invalid movement: Rover would leave the Plateau limits.');
    }
  ...
  private rotate(rotationDirection: RotationDirections) {
    const currentCardinalDirection = RoverNavigationService.cardinalDirectionMap.get(
      this._rover.position.cardinalDirection,
    );

    if (!currentCardinalDirection) {
      throw new Error('Invalid direction: Rover cannot turn in that direction.');
    }

    this._rover.position.cardinalDirection = currentCardinalDirection[rotationDirection];
  }
}
